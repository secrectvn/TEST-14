...
class AccountReports(models.AbstractModel):
    ...
    filter_custom_account_ids = None
    filter_custom_account_type_ids = None
    ...
    def _get_options(self, previous_options=None):
        ...
        self.filter_custom_account_ids = []
        self.filter_custom_account_type_ids = []
    ...
    def _set_context(self, options):
        ...
        if options.get('custom_account_ids'):
            ctx['custom_account_ids'] = self.env['account.account'].browse([int(t) for t in options['custom_account_ids']])
        if options.get('custom_account_type_ids'):
            ctx['custom_account_type_ids'] = self.env['account.account.type'].browse([int(t) for t in options['custom_account_type_ids']])
        return ctx
    ...
    def get_report_informations(self, options):
        ...
        searchview_dict['custom_account_ids'] = [(t.id, t.name) for t in
                                                 self.env['account.account'].search(
                                                     [])] or False
        searchview_dict['custom_account_type_ids'] = [(t.id, t.name) for t in
                                                      self.env['account.account.type'].search([])] or False
            ...
            searchview_dict['analytic_accounts'] = self.env.user.id in self.env.ref(
                'analytic.group_analytic_accounting').users.ids and [(t.id, t.name) for t in
                                                                     self.env['account.analytic.account'].search(
                                                                         [])] or False
            options['selected_analytic_account_names'] = [self.env['account.analytic.account'].browse(int(account)).name for
                                                          account in options['analytic_accounts']]
